...
def run_experiment(exper_body,
           ...
           log_file="log.txt", treatment_names=[], account_list=[]): 
           """
           account_list is a list contain configuration
           """
    PATH="./"+log_file
        ...
        #if response == 'n':
        #    sys.exit(0)
    print('size of account_list:',str(len(account_list)))
    print('size of num_agents:',num_agents)
    fo = open(log_file, "w")
    fo.close()
        ...
        for agent_id in range(0,num_agents):
            procs.append(Process(target=drive_unit, 
                         args = (exper_body,
                             ...
                             log_file, treatment_names,account_list[agent_id],)))
                            # this is username and password
                            #account_list[agent_id] = 'email@domain.it@_@password'
