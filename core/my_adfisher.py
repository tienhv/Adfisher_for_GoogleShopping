...
import driver.my_driver as driver
...
def do_experiment(make_unit, treatments, measurement, end_unit,
    ...
    account_list -- List of accounts that used for login.
    len(account_list) ==  num_units/2. this is a must
    ...
    """
    def exper_body(unit_id, treatment_id, configuration):
        class Test(unittest.TestCase):

            def setUp(self):
                self.unit = make_unit(unit_id, treatment_id, configuration)

            def runTest(self):
                self.unit.log('event', 'progress-marker', "training-start")
                treatments[treatment_id](self.unit, configuration)
                self.unit.log('event', 'progress-marker', "training-end")

                self.unit.wait_for_others()
                
                logging.debug("Waiting for other browser finish")
                self.unit.log('event', 'progress-marker', "measurement-start")
                measurement(self.unit, configuration)
    ...
    if(exp_flag):
        driver.run_experiment(exper_body,
                              ...
                              log_file, treatment_names, account_list)
...
# analysis.statistics.print_frequencies(X, y, features, topk0, topk1)
            p_value = analysis.permutation_test.blocked_sampled_test(
                observed_values, unit_assignments,
            ...
            p_value = analysis.permutation_test.blocked_sampled_test(
                observed_values, unit_assignments, test_stat)
